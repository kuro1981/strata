...
設計対話(Issue 上、または対話セッション)で論点を詰める
   │
   ▼
docs/spec-sml/decisions.sml に "Dn" として裁定を凍結(論点と結論を1行〜数行で記録)
   │
   ▼
...
- 1つの Issue が複数の Dn に分解されることもある(例: `docs/spec-sml/decisions.sml` の
  D58 は Issue #1 から起票された)。
- マイルストーン単位の設計決定・実装の橋渡しは `docs/handoffs/*.sml` に記録する
...
`docs/guides/sml-agent-guide.sml` が正典。これは Strata というプロジェクト自体の開発
