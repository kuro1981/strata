...
//!
//! 同型比較は「ID/Span を消した正規化構造」への変換(`tests/common/mod.rs` の
//! `norm_doc` 以下、WP-F2 で共通化)を経由して行う。無視するのは: 各ブロックの
//! id_tag 全体(alias 含む)、属性行中の `id` エントリ、すべての Span の数値。
//! それ以外(ブロック種別・見出しレベル・リスト項目数・属性行の id 以外のエントリ・
//! インラインの構造とテキスト内容・参照の scheme/target/coord・表の次元木とセル)は
//! 厳密に比較する。
