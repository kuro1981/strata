...
## 1. 導入 {#01J2T8Z1000000000000000000}

[id=01J2T8Z2000000000000000000]
本レポートでは、新たに開発した [予測モデル](term:予測モデル) の性能評価結果について報告する。
評価にあたっては、以下の2つを主要な評価指標（メトリクス）として採用した。

- [予測精度](term:予測精度) — F1スコアを基準とする {#01J2T8Z3000000000000000000}
- [推論速度](term:推論速度) — 推論1回あたりのレイテンシで測る {#01J2T8Z4000000000000000000}

[id=01J2T8Z5000000000000000000, supports=term:予測精度]
予測精度はモデルの実用性を担保するために最も重要な指標であり、今回はF1スコアを基準とする。

## 2. 評価結果（多次元表） {#01J2T8Z6000000000000000000}

[id=01J2T8Z7000000000000000000]
実験における各モデルの評価結果は以下の通りである。

::table {#01J2T8Z8000000000000000000 alias=eval-table}
...
## 3. 分析と考察 {#01J2T8Z9000000000000000000}

[id=01J2T8ZA000000000000000000]
[評価結果の表](table:eval-table) から明らかなように、`Opt-v2` は `Baseline-v1` と比較して大幅な性能向上を達成している。

[id=01J2T8ZB000000000000000000, supports=eval-table]
...
::math {#01J2T8ZD000000000000000000 alias=loss-formula}
L = \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2
::

[id=01J2T8ZE000000000000000000, depends-on=loss-formula]
この [損失関数](math:loss-formula) の計算をGPU向けにカーネル最適化したことが、`Opt-v2` の速度向上の最大の要因である。

::figure {#01J2T8ZF000000000000000000 alias=perf-chart}
