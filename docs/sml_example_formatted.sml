...
- [予測精度](term:予測精度) — F1スコアを基準とする {#01J2T8Z3...}
- [推論速度](term:推論速度) — 推論1回あたりのレイテンシで測る {#01J2T8Z4...}

[id=01J2T8Z5..., supports=term:予測精度]
予測精度はモデルの実用性を担保するために最も重要な指標であり、今回はF1スコアを基準とする。

## 2. 評価結果（多次元表） {#01J2T8Z6...}

[id=01J2T8Z7...]
実験における各モデルの評価結果は以下の通りである。

::table {#01J2T8Z8... alias=eval-table}
...
[id=01J2T8ZB..., supports=eval-table]
...
::math {#01J2T8ZD... alias=loss-formula}
L = \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2
::

[id=01J2T8ZE..., depends-on=loss-formula]
この [損失関数](math:loss-formula) の計算をGPU向けにカーネル最適化したことが、`Opt-v2` の速度向上の最大の要因である。

::figure {#01J2T8ZF... alias=perf-chart}
