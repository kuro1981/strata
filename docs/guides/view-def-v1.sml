---
id: 01KYP2JYDDXEHSQ2JHB9JGSD1T
title: "ビュー定義 v1 文法(D30〜D35)"
alias: view-def-v1
---

# ビュー定義 v1 文法(D30〜D35) {#01KYP2JYDDXEHSQ2JHB9JGSD1V}

[id=01KYP2JYDDXEHSQ2JHB9JGSD1W]
...
(`crates/strata-cli`)。設計決定の出典は[D30](ref:decisions/d30)〜[D35](ref:decisions/d35)。
複数文書入力(`--workspace`)とセレクタの `doc` スコープは M7 WP-W3(D41〜D43、
§3.1.1)で追加。

[id=01KYP2JYDDXEHSQ2JHB9JGSD1X]
...
- `files` の各キーが1つの出力ファイル。`-o <outdir>` と結合される
  ...
  (「各スロットは出力先ファイルを宣言できる」)。 {#01KYP2JYDDXEHSQ2JHB9JGSD27}
- `content:` は1つの**コンビネータ**(§4)。ファイルの中身がオブジェクトなら
  `fields:`、配列なら `rows:` を直接置く。 {#01KYP2JYDDXEHSQ2JHB9JGSD28}
- `manifest:` はこのビュー定義ファイル自身のディレクトリを基準にした相対パス。
  `--check` でのみ使う(§6)。 {#01KYP2JYDDXEHSQ2JHB9JGSD29}

## 3. セレクタ(D31)— グラフのどこを見るか {#01KYP2JYDDXEHSQ2JHB9JGSD2A}

[id=01KYP2JYDDXEHSQ2JHB9JGSD2B]
一級セレクタは4種: **alias** / **class** / **セル座標** / **型+contains パス**。
それぞれ D31 の実測(v0 セレクタ頑健度)に基づく。**正規表現は無い** —
表現できない時は SML 側の構造化が正解、というシグナルとして扱う。

[id=01KYP2JYDDXEHSQ2JHB9JGSD2C]
セレクタは基本的に「マッピング1つに、種類を表すキー1つだけ」の形で書く
(`{alias: basic-info}` のように)。`self` だけは例外で、bare な文字列として書く。

### 3.1 `alias` — もっとも頑健。まず alias を検討する {#01KYP2JYDDXEHSQ2JHB9JGSD2D}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD2E}
{ alias: basic-info }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD2F]
`{#ULID alias=basic-info}` のように SML 側で明示的に付けたエイリアス(D26)から
直接ノードを引く。ファイル内で一意。

#### 3.1.1 `doc` — 文書スコープ修飾(M7 WP-W3、D41〜D43) {#01KYP2JYDDXEHSQ2JHB9JGSD2G}

[id=01KYP2JYDDXEHSQ2JHB9JGSD2H]
`strata view --workspace <strata.toml>` で複数文書を1つのグラフに統合したとき、
`alias` に `doc` を添えると特定の文書のブロック alias を明示的に指せる:

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD2J}
{ alias: licenses, doc: resume }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD2K]
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD2N]
**無指定 doc の解決規則**(裁量): `doc` を省略した無修飾 `alias` は、

[id=01KYP2JYDDXEHSQ2JHB9JGSD2P]
- 単一文書モード(`--workspace` 無し): 従来どおりその1文書内で解決(不変)。 {#01KYP2JYDDXEHSQ2JHB9JGSD2Q}
- ワークスペースモード: その alias 名が**ワークスペース全体で一意**なら
  ...
  起こりうる — 黙ってどちらかを選ばず、`doc:` での明示を要求する)。 {#01KYP2JYDDXEHSQ2JHB9JGSD2R}

[id=01KYP2JYDDXEHSQ2JHB9JGSD2S]
...
### 3.2 `class` — class を持つ唯一のノード {#01KYP2JYDDXEHSQ2JHB9JGSD2T}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD2V}
{ class: marker }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD2W]
そのクラスを持つノードがグラフ全体でちょうど1つの場合に使う。複数あれば
Warning を出しつつ先頭(NodeId 昇順)を使う。0件ならエラー。

### 3.3 `record-field` — record ノードのキーから値を引く {#01KYP2JYDDXEHSQ2JHB9JGSD2X}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD2Y}
{ record-field: { of: { alias: basic-info }, key: 姓 } }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD2Z]
...
### 3.4 `cell` — 表のセル座標(D31 のセル座標セレクタ) {#01KYP2JYDDXEHSQ2JHB9JGSD30}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD31}
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD32]
- `col`: 列キー(セルの `col_path` の先頭セグメント)。 {#01KYP2JYDDXEHSQ2JHB9JGSD33}
- `of`: 対象の表ノードを指すセレクタ。**省略すると、直近の `rows: table`
  (§4.2)が確立した「現在の表」を使う**。 {#01KYP2JYDDXEHSQ2JHB9JGSD34}
- `row`: row_path をリテラルで明示する。**省略すると「現在の row_path」**
  ...
  (上の `tech-stack` の例)に `row` を明示する。 {#01KYP2JYDDXEHSQ2JHB9JGSD35}

### 3.5 `alias-from-row` — 現在の row_path から別ノードへ橋渡しする {#01KYP2JYDDXEHSQ2JHB9JGSD36}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD37}
{ alias-from-row: { prefix: "co-", segment: 0 } }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD38]
...
### 3.6 `first-child-of-type` — 型で絞った最初の子 {#01KYP2JYDDXEHSQ2JHB9JGSD39}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3A}
{ first-child-of-type: { of: { alias: sec-skills-leverage }, type: list } }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3B]
...
### 3.7 `heading-text` — 見出しテキスト一致(Warning 付きエスケープハッチ) {#01KYP2JYDDXEHSQ2JHB9JGSD3C}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3D}
{ heading-text: "タイトル" }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3E]
見出しの平文テキストに一致する Section を探す。**頑健性が低い**(見出しの
言い換えで壊れる)ため、使うたびに Warning を出す。alias を付けられるなら
alias を使うこと(D31)。

### 3.8 `self` — 現在のスコープノード {#01KYP2JYDDXEHSQ2JHB9JGSD3F}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3G}
of: self
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3H]
`rows: contains` の `item`(§4.3)の中で、今まさに反復している子ノード自身を
指す。bare な文字列で書く(唯一の例外)。

## 4. コンビネータ(D32)— 値をどう整形するか {#01KYP2JYDDXEHSQ2JHB9JGSD3J}

[id=01KYP2JYDDXEHSQ2JHB9JGSD3K]
...
details+level 結合)が揃ったため[D45](ref:decisions/d45)で採用された(§4.9)。
文字列テンプレート式(`"{a}({b})"` のような式言語)は D45 でも不採用のまま
(D32 の「式言語への滑り坂」原則、§7 参照)。

### 4.1 `pick` — 値をそのまま取り出す(旧称 `rename`。D35 で改名) {#01KYP2JYDDXEHSQ2JHB9JGSD3M}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3N}
性: { pick: { of: { record-field: { of: { alias: basic-info }, key: 姓 } } } }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3P]
...
```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3Q}
year: { date: { of: { cell: { col: date } }, format: "YYYY", as: int } }
```

#### 4.1.1 糖衣構文: 裸文字列 `alias.キー`(D35) {#01KYP2JYDDXEHSQ2JHB9JGSD3R}

[id=01KYP2JYDDXEHSQ2JHB9JGSD3S]
`pick`+`record-field` の組は実測でビュー定義の**8割**を占める最頻出パターンの
ため、専用の糖衣構文を持つ。値位置に裸文字列 `alias.キー` を書くと、

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3T}
性: basic-info.姓
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3V]
は次の完全形へ機械的に脱糖される:

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD3W}
性: { pick: { of: { record-field: { of: { alias: basic-info }, key: 姓 } } } }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD3X]
- 分割規則: **最初の `.` で** `alias` と `キー` に分ける。alias の字句は
  `[A-Za-z0-9_-]+`([語彙的制約](ref:grammar/lexical-constraints) の key 字句と同じ、ドットを含まない)なので、
  `キー` 側にドットが含まれていても(record のキーは自由テキストで日本語可、
  [D28](ref:decisions/d28))誤分割しない。 {#01KYP2JYDDXEHSQ2JHB9JGSD3Y}
- 糖衣が作るのは常に `as: text` の `pick`(既定と同じ)。数値化したい場合や、
  ...
  1キーを平文抽出する」ケースだけである。 {#01KYP2JYDDXEHSQ2JHB9JGSD3Z}
- **`self` との違い**: セレクタの `self`(§3.8)は「値位置」ではなく `of:` の
  ...
  完全形 `{ pick: { of: self } }` を書くこと。 {#01KYP2JYDDXEHSQ2JHB9JGSD40}
- 糖衣が使えるのはあくまで**コンビネータの値位置**(`fields:` の各エントリの
  右辺、`rows`/`join` の `of:` ではない)。`of:` はセレクタの位置であり、
  `pick` コンビネータそのものが無いため糖衣は展開されない
  (`date`/`age` の `of:` に `alias.キー` は書けない。§3.3 参照)。 {#01KYP2JYDDXEHSQ2JHB9JGSD41}

### 4.2 `rows` — 表 → 行 dict 配列(表モード) {#01KYP2JYDDXEHSQ2JHB9JGSD42}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD43}
content:
  rows:
...
### 4.3 `rows` — ノードの子 → 配列(contains モード) {#01KYP2JYDDXEHSQ2JHB9JGSD46}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD47}
content:
  rows:
...
```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD4C}
projects:
  rows:
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD4D]
SML 側:

``` {#01KYP2JYDDXEHSQ2JHB9JGSD4E}
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD4G]
`rows: table`(§4.2)にはこのフィルタは無い(D58: 行キーは class を
持たないため対象外)。

#### `extend-path` — なぜ必要か、何をするか {#01KYP2JYDDXEHSQ2JHB9JGSD4H}

[id=01KYP2JYDDXEHSQ2JHB9JGSD4J]
...
### 4.4 `join` — 子ノード列・record エントリ列 → 文字列連結 {#01KYP2JYDDXEHSQ2JHB9JGSD4S}

[id=01KYP2JYDDXEHSQ2JHB9JGSD4T]
**木モード**(既定): 子ノードを歩いて連結する。

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD4V}
summary:
  join:
    of: self
    separator: "\n"
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD4W]
`of` が指すノードの直接の子を歩き、`exclude-class`/`include-only-class`
(class フィルタ)で絞りながら:

[id=01KYP2JYDDXEHSQ2JHB9JGSD4X]
- リスト以外の子(段落など)は、その本文テキストをそのまま1行にする。 {#01KYP2JYDDXEHSQ2JHB9JGSD4Y}
- リスト型の子は、その各項目を1行にし、さらに項目がネストしたリストを
  持てば、その孫項目を `nested-prefix` を前置して1行ずつ追加する
  (「トップ項目1つ + ネストされた内訳」という職務経歴の記法パターンに
  対応)。 {#01KYP2JYDDXEHSQ2JHB9JGSD4Z}

[id=01KYP2JYDDXEHSQ2JHB9JGSD50]
`separator` で行を連結する。

[id=01KYP2JYDDXEHSQ2JHB9JGSD51]
**record モード**(`keys` を指定): `of` が record ノードのとき、指定した
キーだけを宣言順に `"キー: 値"` 形式の行にし(値が空文字列のキーは省略)、
`separator` で連結する。

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD52}
志望動機:
  join:
    of: { alias: other-info }
    separator: "\n\n"
...
### 4.5 `date` — Date/Period → 書式文字列 {#01KYP2JYDDXEHSQ2JHB9JGSD53}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD54}
生年月日:
  date:
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD58]
`of` が `Text`(型付きパースに失敗しテキストへフォールバックした値。例:
複数区間 `"2020-09 ~ 2021-03 / 2021-09 ~ ..."`)ならそのまま通す。

[id=01KYP2JYDDXEHSQ2JHB9JGSD59]
`as: int` を付けると結果を整数として出力する(既定 `as: text`)。

### 4.6 `age` — 生年月日+基準日 → 満年齢 {#01KYP2JYDDXEHSQ2JHB9JGSD5A}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5B}
満年齢:
  age:
...
### 4.7 `literal` — 固定値 {#01KYP2JYDDXEHSQ2JHB9JGSD5D}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5E}
environments: { literal: "Linux, Windows, macOS" }
tech_skills: { literal: [] }
Email2: { literal: null }
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD5F]
グラフに対応するデータが無いスロット(v0 の環境フォールバック値・案内文言
など)や、常に空であるべきスロット(第2連絡先など)を**明示的に**宣言する。
「バインディングはデータを持たない」原則に反して残っていた v0 の暗黙の
デフォルトを、ここで宣言として可視化する([decisions.sml](ref:decisions/view-v1) D30〜D34 節末尾の
既知の注意点)。

### 4.8 `fields` — 名前付きサブコンビネータの集まり {#01KYP2JYDDXEHSQ2JHB9JGSD5G}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5H}
fields:
  性: basic-info.姓 # 糖衣(§4.1.1)。完全形なら { pick: { ... } }
  名: basic-info.名
```

[id=01KYP2JYDDXEHSQ2JHB9JGSD5J]
`rows` の `item` や、ファイルの `content` 直下で使う「まとめ役」。**YAML の
宣言順がそのまま出力の列順になる**(決定性・可読性のため、辞書順には
並び替えない)。

### 4.9 `concat` — 複数コンビネータの文字列連結(D45) {#01KYP2JYDDXEHSQ2JHB9JGSD5K}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5M}
name:
  concat:
    parts:
      - resume/basic-info.姓
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD5P]
`literal` と組み合わせて、値と値の間に固定の記号(改行・括弧等)を挟める:

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5Q}
languages:
  concat:
    parts:
...
[id=01KYP2JYDDXEHSQ2JHB9JGSD5R]
`cv-jis.view.yaml`(`~/dev/strata-my-resume/sml/views/`)の実例2件:

[id=01KYP2JYDDXEHSQ2JHB9JGSD5S]
- **氏名の姓名結合**(`build_cv/content/profile.yaml.fields.name`): M7 で
  ...
  区切り)+ `resume/basic-info.名` へ置き換えた。 {#01KYP2JYDDXEHSQ2JHB9JGSD5T}
- **tech-stack の details(level) 復元**(`experience.languages`/`frameworks`/
  ...
  `literal: ")"`)で宣言的に再現した。 {#01KYP2JYDDXEHSQ2JHB9JGSD5V}

[id=01KYP2JYDDXEHSQ2JHB9JGSD5W]
...
## 5. profile(D34) {#01KYP2JYDDXEHSQ2JHB9JGSD5X}

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD5Y}
...
## 6. マニフェストと `--check`(D33) {#01KYP2JYDDXEHSQ2JHB9JGSD60}

[id=01KYP2JYDDXEHSQ2JHB9JGSD61]
マニフェストはテンプレートが実際に読むスロット(ファイル+フィールド)の
宣言。ビュー定義から `manifest:` で参照する。

```yaml {#01KYP2JYDDXEHSQ2JHB9JGSD62}
version: 1
files:
...
## 7. 既知の限界(v1 スコープ外・拡張裁定候補) {#01KYP2JYDDXEHSQ2JHB9JGSD69}

[id=01KYP2JYDDXEHSQ2JHB9JGSD6A]
- ~~**クロスドキュメント合成不可**~~(→ M7 WP-W3 で解消。`strata view --workspace
  ...
  (→ M7.5 WP-Z1 で解消。[D44](ref:decisions/d44): `render --workspace [--doc]`・
  `context --workspace [--doc]` を追加。MD のページ間リンクは相対 `.md` リンク+
  アンカー、Typst は文書名付き退化テキスト。詳細は [workspace-m75-handoff](doc:workspace-m75-handoff))。 {#01KYP2JYDDXEHSQ2JHB9JGSD6B}
- ~~**多フィールドの自由な連結(テンプレート文字列)は書けない**~~(→ D45 で
  ...
  持たない(D32「式言語への滑り坂」原則の一貫)。 {#01KYP2JYDDXEHSQ2JHB9JGSD6C}

## 8. 実例 {#01KYP2JYDDXEHSQ2JHB9JGSD6D}

[id=01KYP2JYDDXEHSQ2JHB9JGSD6E]
完全な実例は次の2ファイル(`~/dev/strata-my-resume/sml/views/`)を参照:

[id=01KYP2JYDDXEHSQ2JHB9JGSD6F]
- `resume-jis.view.yaml` — JIS 履歴書(1 profile: submit)。record/表からの
  素朴な pick(大半は `alias.キー` 糖衣)・date・age・literal の使用例。 {#01KYP2JYDDXEHSQ2JHB9JGSD6G}
- `cv-jis.view.yaml` — 職務経歴書(2 profile: submit/check)。`rows: contains` {#01KYP2JYDDXEHSQ2JHB9JGSD6H}
  + `extend-path` による3方向 join、`join` の木モード/record モード両方、
  profile 分岐の実例。 {#01KYP2JYDDXEHSQ2JHB9JGSD6J}

[id=01KYP2JYDDXEHSQ2JHB9JGSD6K]
