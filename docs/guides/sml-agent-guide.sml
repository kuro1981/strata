---
id: 01KYP2JYCKM84M4A3NM14X9RQC
title: "SML 執筆ガイド(AI エージェント向け)"
alias: sml-agent-guide
---

# SML 執筆ガイド(AI エージェント向け) {#01KYP2JYCKM84M4A3NM14X9RQD}

[id=01KYP2JYCKM84M4A3NM14X9RQE]
対象読者: SML(Strata Markup Language)文書を**書く・編集する AI エージェント**
(Claude に限らない)。このガイド**だけ**を読めば SML を正しく書けることを
品質基準とする([D37](ref:decisions/d37))。記法の正典は[grammar.sml](doc:grammar)+[decisions.sml](doc:decisions)、ビュー
定義の正典は [view-def-v1](doc:view-def-v1)。細部で迷ったら本書ではなくそちらを
見ること——本書は実用要約であり、食い違えばそちらが正しい。

## 0. 全体の流れ {#01KYP2JYCKM84M4A3NM14X9RQF}

``` {#01KYP2JYCKM84M4A3NM14X9RQG}
...
### 1.3 リスト(ネスト可) {#01KYP2JYCKM84M4A3NM14X9RQT}

```markdown {#01KYP2JYCKM84M4A3NM14X9RQV}
- トップ項目 {#01KXxxx1}
...
[id=01KYP2JYCKM84M4A3NM14X9RQW]
- 項目=段落1つの制約は維持(1項目に複数ブロックは書けない)。子リストのネストは可 {#01KYP2JYCKM84M4A3NM14X9RQX}
- 長い項目は折り返してよい(D52、CommonMark 準拠の継続行): マーカー行の直後に
  空行を挟まず続く行は、同じ項目の本文として併合される(新規マーカー行が来た
  時点でそこは子項目/次の項目になる)。`{#id}` タグは**最終行**の末尾に付く: {#01KYP2JYCKM84M4A3NM14X9RQY}

```markdown {#01KYP2JYCKM84M4A3NM14X9RQZ}
- 長い説明が折り返して
  次の行に続く1項目 {#01KXxxx5}
```

[id=01KYP2JYCKM84M4A3NM14X9RR0]
- リスト全体に ID/意味エッジを付けたいときだけ前置属性行を書く: {#01KYP2JYCKM84M4A3NM14X9RR1}

```markdown {#01KYP2JYCKM84M4A3NM14X9RR2}
[id=my-list, supports=some-target]
- 項目1 {#...}
- 項目2 {#...}
```

### 1.4 属性行 `[key=value, ...]` {#01KYP2JYCKM84M4A3NM14X9RR3}

[id=01KYP2JYCKM84M4A3NM14X9RR4]
直後のブロックに束縛される独立行。使えるキー:

[id=01KYP2JYCKM84M4A3NM14X9RR5]
| キー | 意味 |
|---|---|
...
[id=01KYP2JYCKM84M4A3NM14X9RR6]
上記以外のキーは診断 `UnknownAttrKey`(Warning、ファイルは書き換わるがタイポの
可能性として警告される)。

```markdown {#01KYP2JYCKM84M4A3NM14X9RR7}
[id=key-finding, supports=eval-table, class=note]
このブロックが eval-table を支持することを宣言しつつ note 分類も付ける段落。
```

[id=01KYP2JYCKM84M4A3NM14X9RR8]
**[D46](ref:decisions/d46)(2026-07-15 裁定)**: 実効 class は「自身+祖先
...
```markdown {#01KYP2JYCKM84M4A3NM14X9RR9}
[class=note]
...
### 1.5 フロントマター(任意) {#01KYP2JYCKM84M4A3NM14X9RRB}

```markdown {#01KYP2JYCKM84M4A3NM14X9RRC}
---
...
[id=01KYP2JYCKM84M4A3NM14X9RRD]
- キーは `id`(ULID)・`title`(自由文字列)・`alias`(文書エイリアス、
  ワークスペースの横断参照 `ref:<文書alias>/<...>` の左辺。D41)・`class`
  (D46 の実効 class。文書全体に効く — 直下の全ブロックに自動継承される。
  D61 で追加)の4つ。他のキーは診断(未知キー扱い) {#01KYP2JYCKM84M4A3NM14X9RRE}
- 通常は `strata fmt` が既存ファイルに無ければ自動生成する。あなたが手で
  `id:` を書く必要はない(書くなら ULID のみ。ラベルは書けない) {#01KYP2JYCKM84M4A3NM14X9RRF}

### 1.6 `::table` — 多次元表 {#01KYP2JYCKM84M4A3NM14X9RRG}

```markdown {#01KYP2JYCKM84M4A3NM14X9RRH}
::table {#eval-table}
...
[id=01KYP2JYCKM84M4A3NM14X9RRJ]
- `- <次元名>:` の直下が member。`- <次元名>: [k1, k2]` はフラット次元の糖衣 {#01KYP2JYCKM84M4A3NM14X9RRK}
- member をネストすると多層の行/列次元になる(会社×プロジェクトのような二階層) {#01KYP2JYCKM84M4A3NM14X9RRM}
- member に表示名を付けたい時: `- key "表示ラベル"`(例: `- languages "プログラミング言語"`) {#01KYP2JYCKM84M4A3NM14X9RRN}
- セル座標は `行path | 列path : 値`。path は `key("."key)*`(ネスト次元はドット連結、
  例 `dentsu.isid_azure_ml_prod`)。`|`/`.`/`:` の前後の空白は無視されるので
  桁揃えしてよい {#01KYP2JYCKM84M4A3NM14X9RRP}
- セル値の型付きパース: 裸の数値 → Number、`<数値> <単位>`(例 `12 ms`) →
  Quantity(構造化)、`"..."` またはそれ以外の裸テキスト → Text、`~`/空 → Empty、
  `ref:<target>` → Ref {#01KYP2JYCKM84M4A3NM14X9RRQ}
- 日付・期間セルは §1.9 参照 {#01KYP2JYCKM84M4A3NM14X9RRR}
- `[caption=...]`、フィギュアの `[depicts=...]` はフェンス直後の**フェンス内属性行**
  に書く(この位置に `id=` は書けない。ID はマーカー行の `{#...}` のみ) {#01KYP2JYCKM84M4A3NM14X9RRS}

### 1.7 `::record` — キー・値ブロック {#01KYP2JYCKM84M4A3NM14X9RRT}

```markdown {#01KYP2JYCKM84M4A3NM14X9RRV}
...
[id=01KYP2JYCKM84M4A3NM14X9RRW]
- 本体は「キー: 値」の行の列。**キーは自由テキスト(日本語可)** — 表の座標キー
  (ASCII 限定)とは別物 {#01KYP2JYCKM84M4A3NM14X9RRX}
- 値の型付きパースは表セルと同じ(Number/Quantity/Date/Period/Text/Ref) {#01KYP2JYCKM84M4A3NM14X9RRY}
- 同一キーの重複宣言は診断 `DuplicateRecordKey`(Warning。値は失われず全件残る) {#01KYP2JYCKM84M4A3NM14X9RRZ}

### 1.8 `::math` — ブロック数式 {#01KYP2JYCKM84M4A3NM14X9RS0}

```markdown {#01KYP2JYCKM84M4A3NM14X9RS1}
::math {#loss-formula}
L = \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2
::
```

[id=01KYP2JYCKM84M4A3NM14X9RS2]
本文は TeX。未対応の綴りは build 時にエラーになる(「出たら足す」方針、正典は
[strata-spec](doc:strata-spec) §6)。

### 1.9 `::figure` — 図 {#01KYP2JYCKM84M4A3NM14X9RS3}

[id=01KYP2JYCKM84M4A3NM14X9RS4]
記号図(チャート、データを焼かない):

```markdown {#01KYP2JYCKM84M4A3NM14X9RS5}
::figure {#perf-chart}
...
[id=01KYP2JYCKM84M4A3NM14X9RS6]
写真(画素に意味がある):

```markdown {#01KYP2JYCKM84M4A3NM14X9RS7}
::figure {#photo1}
...
### 1.10 インライン参照(ナビゲーション、意味エッジは張らない) {#01KYP2JYCKM84M4A3NM14X9RS8}

``` {#01KYP2JYCKM84M4A3NM14X9RS9}
[表示テキスト](<scheme>:<target>)
```

[id=01KYP2JYCKM84M4A3NM14X9RSA]
| scheme | 意味 |
|---|---|
| `ref:` | 任意ブロックへの汎用参照 |
...
| `cell:` | 表の特定セルへの参照: `[12 ms](cell:eval-table#Opt-v2\|Dataset-A.Latency)`(GFM 表セル内のためパイプはエスケープ表記) |
...
[id=01KYP2JYCKM84M4A3NM14X9RSB]
- target は ULID でもエイリアスでもよい。target の前に `#` を付けない(`#` は
  `cell:` の座標区切り専用) {#01KYP2JYCKM84M4A3NM14X9RSC}
- 論証関係(supports 等)を張りたい場合はインライン参照ではなく §1.4 の属性行を使う {#01KYP2JYCKM84M4A3NM14X9RSD}
- 外部リンクは通常の Markdown 記法で書ける(M6/D40): `[text](https://…)`・
  `mailto:`・autolink `<https://…>`・画像 `![alt](url)`。エスケープ(`\*` 等)・
  取消線 `~~x~~`・GFM パイプ表・タスクリスト `- [ ]` も対応済み。
  `![alt](ref:...)`(内部参照の画像化)は未対応で明示エラーになる {#01KYP2JYCKM84M4A3NM14X9RSE}
- 別ファイルのブロックへの参照はワークスペース(D41〜D42)で書ける:
  `[text](ref:<文書alias>/<ブロックalias>)`(文書 alias はフロントマターで宣言)。
  これを含む文書の build/render は `--workspace <strata.toml>` が必要 {#01KYP2JYCKM84M4A3NM14X9RSF}

### 1.11 Date・Period のセル/record 値 {#01KYP2JYCKM84M4A3NM14X9RSG}

[id=01KYP2JYCKM84M4A3NM14X9RSH]
既定の受理書式は **ISO のみ**(`YYYY-MM-DD` または `YYYY-MM`)。それ以外の書式
(例: `2026年7月`)を書きたい場合は、そのフェンスの属性行に `date-format=` で
明示宣言する:

```markdown {#01KYP2JYCKM84M4A3NM14X9RSJ}
...
## 2. D37 の作法(正典。ここが本書の核) {#01KYP2JYCKM84M4A3NM14X9RSM}

[id=01KYP2JYCKM84M4A3NM14X9RSN]
[D37](ref:decisions/d37) で確定した、AI が SML を書く際の作法。以下5点は**必ず守る**:

[id=01KYP2JYCKM84M4A3NM14X9RSP]
1. **ULID を書かない・発明しない**。ID の発行は `strata fmt` の仕事。ドラフトでは
   `{#ラベル}`(行型)か無記名でよい。それらしい ULID 風の文字列を捏造しない {#01KYP2JYCKM84M4A3NM14X9RSQ}
2. **alias を積極的に付ける**。意味のある名前(例: `proj-new-thing`)を選ぶ。
   ビューからも人からも引ける「アドレス」になる([view-def-v1](doc:view-def-v1) の
   セレクタは alias を最も頑健な参照手段として使う) {#01KYP2JYCKM84M4A3NM14X9RSR}
3. **既存ノードへの参照・エッジは `strata context` 出力のアドレスタグから
   コピーする**。手で ULID や alias を書き写す/推測しない(§3 参照)。タグの
   記法は SML と同一(`{#ULID}` / `{#ULID alias=x}`)なのでそのまま貼り付けられる {#01KYP2JYCKM84M4A3NM14X9RSS}
4. **エッジは確信のあるものだけ張る。推測で張らない**。`supports` /
   ...
   ありそうだが確信が持てないため未実装」) {#01KYP2JYCKM84M4A3NM14X9RST}
5. **AI 下書きの専用 class は付けない**(例: `class=ai-draft` のようなタグを
   ...
   指示が承認ゲート) {#01KYP2JYCKM84M4A3NM14X9RSV}

### 2.1 既存文書へ追記する時の追加作法(2026-07-15 批准追記) {#01KYP2JYCKM84M4A3NM14X9RSW}

[id=01KYP2JYCKM84M4A3NM14X9RSX]
6. **挿入位置は既存の並び順規約に従う**(時系列・グループ順など、context で
   観察して読み取る)。規約が読み取れない・確信が持てない場合は**末尾に追記**し、
   その判断を最終報告に書く {#01KYP2JYCKM84M4A3NM14X9RSY}
7. **集計・要約ブロックは変更しない**。概要表(career-overview 等)・職務要約の
   ような「他ブロックの内容を集計・要約したノード」は、詳細を追記しても
   自分では書き換えず、**更新候補として最終報告で提案**する(作法4のエッジと
   同じ確信原則の適用 — 要約の意図は著者にしか分からない) {#01KYP2JYCKM84M4A3NM14X9RSZ}
8. **alias の命名は既存パターンを踏襲する**。`strata context` で同種ブロックの
   alias を観察し、同じ接頭辞・語彙・区切り文字で命名する(文書ごとの規約が
   一般規約に優先する) {#01KYP2JYCKM84M4A3NM14X9RT0}

---

## 3. 書く前に: グラフを読む {#01KYP2JYCKM84M4A3NM14X9RT1}

[id=01KYP2JYCKM84M4A3NM14X9RT2]
...
```bash {#01KYP2JYCKM84M4A3NM14X9RT3}
# 1. 文書全体を俯瞰する
...
[id=01KYP2JYCKM84M4A3NM14X9RT4]
- `--node` は複数指定可。既定 `--hops` は1 {#01KYP2JYCKM84M4A3NM14X9RT5}
- 出力をファイルに保存したい場合は `-o out.md` {#01KYP2JYCKM84M4A3NM14X9RT6}
- 存在しない alias/ULID を指定すると exit 2 で明確なエラーになる {#01KYP2JYCKM84M4A3NM14X9RT7}
- 出力末尾の「エッジ」節に `supports` / `depends-on` / `cites` / `refers-to` /
  `term-ref` の一覧が出る(`contains` は見出し構造そのものに現れるので出ない)。
  新しいエッジを張る前に、対象との重複が無いかここを確認する {#01KYP2JYCKM84M4A3NM14X9RT8}

[id=01KYP2JYCKM84M4A3NM14X9RT9]
`--node` で対象サブツリーを見た後、**既存のセクション(会社・プロジェクト等)と
同じ形に揃えて**新しいブロックを書くのが最も安全(既存パターンの模倣)。

### 3.1 grep の代わりに `strata search` を使う {#01KYP2JYCKM84M4A3NM14X9RTA}

[id=01KYP2JYCKM84M4A3NM14X9RTB]
編集対象や参照先を**文字列で**探したい時(「この用語を使っている箇所は他にあるか」
「`eval-` で始まる alias にはどんなものがあるか」等)は、生ファイルへの `grep` では
なく `strata search` を使う([D56](ref:decisions/d56))。grep と違い、構造述語・
実効 class(D46: 自身+祖先の和集合)・人間可読ラベル・マッチ箇所のスニペットが
最初から付いて返ってくる:

```bash {#01KYP2JYCKM84M4A3NM14X9RTC}
...
### exit code の意味 {#01KYP2JYCKM84M4A3NM14X9RTH}

[id=01KYP2JYCKM84M4A3NM14X9RTJ]
| コマンド | 0 | 1 | 2 |
|---|---|---|---|
...
[id=01KYP2JYCKM84M4A3NM14X9RTV]
1行目のような未解決参照は alias のタイポが典型原因(context 出力からコピーし
直す)。3行目のような Warning は exit 0 だが、たいていタイポのサインである。

---

## 5. チェックリスト(書き終えたら) {#01KYP2JYCKM84M4A3NM14X9RTW}

[id=01KYP2JYCKM84M4A3NM14X9RTX]
- [ ] ULID を自分で書いていないか(捏造していないか) {#01KYP2JYCKM84M4A3NM14X9RTY}
- [ ] 新規ブロックに意味のある alias を付けたか {#01KYP2JYCKM84M4A3NM14X9RTZ}
- [ ] 既存ノードへの参照・エッジは `strata context` 出力のタグからコピーしたか(手打ちで推測していないか) {#01KYP2JYCKM84M4A3NM14X9RV0}
- [ ] 張ったエッジ(`supports`/`depends-on`/`cites`)はすべて確信があるか(迷ったものは張らずに報告へ) {#01KYP2JYCKM84M4A3NM14X9RV1}
- [ ] AI 下書き専用の class を付けていないか {#01KYP2JYCKM84M4A3NM14X9RV2}
- [ ] `strata fmt <file>` を実行し、再実行しても差分が出ない(冪等)ことを確認したか {#01KYP2JYCKM84M4A3NM14X9RV3}
- [ ] `strata build <file>` の診断がゼロ(Error 無し)であることを確認したか。Warning が出た場合は内容を確認したか {#01KYP2JYCKM84M4A3NM14X9RV4}
- [ ] (ビュー定義があるプロジェクトなら)`strata view --check` を実行したか {#01KYP2JYCKM84M4A3NM14X9RV5}
- [ ] 推測で張らずに見送ったエッジ候補があれば、最終報告に人間への提案として書いたか {#01KYP2JYCKM84M4A3NM14X9RV6}
