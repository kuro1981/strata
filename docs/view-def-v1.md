...
- **クロスドキュメント合成不可**: `strata view` は1 SML ファイル=1グラフが
  入力(D30)。複数 SML ファイルにまたがる合成(例: 履歴書の氏名を職務経歴書の
  ヘッダに転記する)は表現できない。回避策は「必要な最小限のデータを両方の
  SML に持たせる」(DRY 違反を最小化した上での複製)か、「1つのビュー定義が
  他のテンプレートのディレクトリへ出力ファイルを書く」(§2 の「各スロットは
  出力先ファイルを宣言できる」を使う)のいずれか。ファイル横断のエイリアス表
  (sml-spec.md §10 で保留)が入れば根本解決する。
