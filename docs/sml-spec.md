...
- 行型ブロック(見出し・リスト・フェンス)の前置属性行に `id=` を書くとエラー。
  自身の `{#...}` タグとの併記なら「重複」、単独でも「置き場所違反」として診断される
- **リスト**は前置属性行がリスト全体に束縛される一方、ID は項目ごとの行内 `{#id}` が
  担うため、リストへの属性行 `id=` は常に置き場所違反
...
- **コードフェンス(``` )の ID 記法** — §2 のブロック分類に未登場。行型として
  開始行末尾に `{#id}` を許すのが一貫的だが未決。現状 ID を持てず、build 時に
  不安定 ID になる問題がある(不変条件1に抵触しうる)
